        ...
        for enquiry_number in pay_rec.enquiryList:
            ...
            if enquiry:
                ...
                if pay_rec.paymentType == u'DEP' and \
                        (enquiry.getStateName() in ['allocated', 
                                                  'detailsreceieved']):
                    ...
                    elif enquiry.getStateName() in ['allocated', 'detailsreceieved']: 
                        transition_name = 'payall'
                    txn_description = 'Payment REF %s: Outstanding Balance\n' % \
                                                        (enquiry_number)
                else:
                    raise WrongEnquiryStateException, 'Wrong enquiry state'
        ...
        if not pay_rec.duplicateTransaction and \
            ...
            try:
                ...
                self._addErrorNode(node, code='305', 
                        message='No credit card holder on enquiry batch')
                self.response.headers['Content-Type'] = 'text/plain'
                self.response.out.write(tostring(node))
                return
