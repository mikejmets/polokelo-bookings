...
from controllers.provider import ManageProviders, \
    ViewProvider, CaptureProvider, EditProvider, DeleteProvider
from controllers.service import ViewService, \
    CaptureService, EditService, DeleteService
...
PAGESIZE = 15
class ManageServices(webapp.RequestHandler):
    def get(self):

        auth_url, auth_url_text = get_authentication_urls(self.request.uri)

        filepath = os.path.join(PROJECT_PATH, 'templates', 'services', 'manageservices.html')
...
application = webapp.WSGIApplication([
                ...
                ('/services/providers', ManageProviders),
                ('/services/provider/viewprovider', ViewProvider),
                ('/services/provider/captureprovider', CaptureProvider),
                ('/services/provider/editprovider', EditProvider),
                ('/services/provider/deleteprovider', DeleteProvider),
                ('/services/provider/address/captureaddress', CaptureAddress),
                ('/services/provider/address/editaddress', EditAddress),
                ('/services/provider/address/deleteaddress', DeleteAddress),
                ('/services/provider/contact/capturephone', CapturePhoneNumber),
                ('/services/provider/contact/editphone', EditPhoneNumber),
                ('/services/provider/contact/deletephone', DeletePhoneNumber),
                ('/services/provider/email/captureemail', CaptureEmailAddress),
                ('/services/provider/email/editemail', EditEmailAddress),
                ('/services/provider/email/deleteemail', DeleteEmailAddress),
                ('/services/provider/viewservice', ViewService),
                ('/services/provider/captureservice', CaptureService),
                ('/services/provider/editservice', EditService),
                ('/services/provider/deleteservice', DeleteService),
